...
// 1. DATE SETTINGS (Adjust these as needed)
const gMomBirth = new Date("1945-05-12"); 
const dadBirth = new Date(gMomBirth);
dadBirth.setFullYear(dadBirth.getFullYear() + 18); // Father born when she was 18

const myBirth = new Date(dadBirth);
myBirth.setFullYear(myBirth.getFullYear() + 20); // I was born when he was 20
...
// 2. GRID SETTINGS
const itemsPerRow = 60; // Line breaks every 60
const xSpacing = 32;    // Increased width to prevent horizontal cutoff
const ySpacing = 75;    // Vertical spacing
const tallies = [];
...
// 5. RESIZE & PADDING LOGIC
function resizeWall() {
    if (tallies.length === 0) return;
    const lastTally = tallies[tallies.length - 1];
    
    const totalHeight = lastTally.y + 200;
    // Added 180px extra to totalWidth to act as a right-side border/margin
